...
enum MixType {
	MIX_SPHERE, MIX_ELLIPSOID, MIX_DOUBLESPHERE, MIX_CUBE, MIX_CUBE_SPHERE, MIX_CYLINDER, MIX_CONE, MIX_TYPE1, MIX_TYPE2, MIX_TYPE3, MIX_TYPE4
};
...
class ParticleGeneratorCPU {
	public:
		ParticleGeneratorCPU(ChSystem* system) {
			mSys = system;
			mass = 1;
			use_normal_dist = false;
			use_density = false;
		...
		void createBody(ChSharedBodyPtr body, real3 pos, real mass) {
			...
			if (use_common_material) {
				...
				InitObject(body, mass, Vector(pos.x, pos.y, pos.z), Quaternion(1, 0, 0, 0), true, false, 2, 4);
				if (use_normal_friction) {
					body->GetMaterialSurface()->SetFriction(distribution_friction->operator()(generator));
				} else {
					body->GetMaterialSurface()->SetFriction(material->GetSfriction());
