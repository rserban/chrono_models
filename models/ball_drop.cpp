...
int main(int argc, char* argv[])
{
	...
	double end_time=time_step;//0.2; //0.05
...
mcollisionengine->broadphase.setBinsPerAxis(R3(200, 200, 200));
	...
	curr_particles+=add_particles_fromFile_V2(mSys, num_to_load, prad, pmass, mu, "input_data/SpherePos_500K_APGD_v1.dat");
...
GetLog() << "Residual: " << ((ChLcpSolverGPU *) (mSys->GetLcpSolverSpeed()))->GetResidual() << "\n";
		GetLog() << "ITER: " << ((ChLcpSolverGPU *) (mSys->GetLcpSolverSpeed()))->GetTotalIterations() << "\n";
			GetLog() << "OUTPUT STEP: Time= "<< current_time <<" bodies= "<< mSys->GetNbodies() << " contacts= " << mSys->GetNcontacts() << " step time=" << mSys->GetTimerStep() << " lcp time=" << mSys->GetTimerLcp()  << " CDbroad time=" << mSys->GetTimerCollisionBroad() << " CDnarrow time=" << mSys->GetTimerCollisionNarrow() << " Iterations=" << ((ChLcpSolverGPU*) (mSys->GetLcpSolverSpeed()))->GetTotalIterations() << "\n";
