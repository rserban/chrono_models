...
void InitObject(ChSharedBodyPtr &body, double mass, ChVector<> pos, ChQuaternion<> rot, ChSharedPtr<ChMaterialSurface> &material, bool do_collide = true, bool is_fixed = false,
		int collision_family = 2, int do_not_collide_with = 4) {
	body->SetMass(mass);
	body->SetPos(pos);
	body->SetRot(rot);
...
void InitObject(ChSharedBodyPtr &body, double mass, ChVector<> pos, ChQuaternion<> rot, bool do_collide = true, bool is_fixed = false, int collision_family = 2,
...
void AddCollisionGeometry(ChSharedBodyPtr &body, ShapeType type, ChVector<> dimension, ChVector<> local_position = ChVector<>(0, 0, 0), ChQuaternion<> local_rotation =
...
void AddCollisionGeometryTriangleMesh(ChSharedBodyPtr &body, string file_name, ChVector<> local_position = ChVector<>(0, 0, 0),
		ChQuaternion<> local_rotation = ChQuaternion<>(1, 0, 0, 0)) {
	ChMatrix33<> rotation = local_rotation;
	ChCollisionModel *model = (ChCollisionModel *) body->GetCollisionModel();
