...
int main(int argc, char* argv[]) {
	if (argc == 5) {
		...
		cout << "Freq: = " << frequency << " Amplitude: = " << amplitude << endl;
		seconds_to_simulate = atof(argv[3]);
	...
	((ChLcpSolverParallelDVI *) (system_gpu->GetLcpSolverSpeed()))->SetContactRecoverySpeed(ceil(2 * PI * frequency * amplitude)*2);     //IMPORTANT: this is the max velocity of the plate!!
	...
	((ChCollisionSystemParallel *) (system_gpu->GetCollisionSystem()))->setBinsPerAxis(I3(num_per_dir.x / 2, num_per_dir.y / 2, num_per_dir.z / 2));
	...
	FinalizeObject(Bottom, (ChSystemParallelDVI *) system_gpu);
	//=========================================================================================================
	...
	layer_gen.addVolume(R3(0, 2, 0), num_per_dir, R3(0, 0, 0));
...
//	window_manager->CallGlutMainLoop();
	...
	for (int i = 0; i < num_steps; i++) {
		...
		cout << "step " << i;
		cout << " Residual: " << ((ChLcpSolverParallelDVI *) (system_gpu->GetLcpSolverSpeed()))->GetResidual();
		cout << " ITER: " << ((ChLcpSolverParallelDVI *) (system_gpu->GetLcpSolverSpeed()))->GetTotalIterations();
		cout << " OUTPUT STEP: Time= " << current_time << " bodies= " << system_gpu->GetNbodies() << " contacts= " << system_gpu->GetNcontacts() << " step time="
				...
				<< system_gpu->GetTimerCollisionNarrow() << " Iterations=" << ((ChLcpSolverParallelDVI*) (system_gpu->GetLcpSolverSpeed()))->GetTotalIterations() << "\n";
