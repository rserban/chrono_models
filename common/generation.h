...
void addPerturbedLayer(
	...
	ChSharedBodyPtr body;
	int counter = 0;
	ChSharedPtr<ChMaterialSurface> matsurface = ChSharedPtr<ChMaterialSurface>(new ChMaterialSurface);

	matsurface->SetCohesion(cohesion);
	matsurface->SetCompliance(compliance);
	matsurface->SetFriction(mu);
	for (int i = 0; i < num_per_dir.x; i++) {
		for (int j = 0; j < num_per_dir.y; j++) {
			for (int k = 0; k < num_per_dir.z; k++) {
				real3 r = rad;
...
//				}
				...
				body = ChSharedBodyPtr(new ChBody(new ChCollisionModelGPU));
				body->SetMaterialSurface(matsurface);
				dp.x = rand() % 10000 / 10000.0 * a.x - a.x / 2.0;
				dp.y = rand() % 10000 / 10000.0 * a.y - a.y / 2.0;
				dp.z = rand() % 10000 / 10000.0 * a.z - a.z / 2.0;
				//d = d * .6;
